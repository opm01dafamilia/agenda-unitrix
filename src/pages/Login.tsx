...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.error) {
        log("Google login error", result.error);
        toast.error("Falha ao entrar com Google. Tente novamente.");
      }
      // If redirected, page will reload and AuthContext handles the rest
    } catch (err: any) {
      log("Google login catch", err);
      ...
      <div className="w-full max-w-sm animate-fade-in">
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={loading || googleLoading}>{loading ? "Entrando..." : "Entrar"}</Button>
          </form>
        </div>
