...
const DashboardLayout = () => {
    ...
    <nav className="flex flex-col gap-1 p-4">
      {navItems.map((item) => (
        <Link key={item.path} to={item.path} onClick={() => setSheetOpen(false)}
          className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${isActive(item.path) ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:text-foreground hover:bg-accent"}`}>
  ...
  return (
    <div className="min-h-screen bg-background">
      {showPastDueBanner && (
        <div className="bg-destructive/20 text-destructive text-sm text-center py-2 px-4 font-medium">
          ⚠️ Você está com pendência na mensalidade. Pague em até 7 dias para não perder o acesso.
        </div>
      )}
      ...
      <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-64 border-r border-border/50 flex-col bg-sidebar">
        <div className="p-4 border-b border-border/50">
          ...
          {business && <p className="text-xs text-muted-foreground mt-1 truncate">{business.name}</p>}
        </div>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-14 border-b border-border/50 bg-background/80 backdrop-blur-xl z-50 flex items-center justify-between px-4">
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          ...
          <SheetContent side="left" className="w-64 p-0">
            <div className="p-4 border-b border-border/50">
              <span className="text-xl font-bold tracking-tight"><span className="text-primary">IA</span> agenda</span>
            </div>
            <NavLinks />
            <div className="p-4 border-t border-border/50">
              <Button variant="ghost" className="w-full justify-start text-muted-foreground" size="sm" onClick={signOut}>
        ...
        <div className="flex justify-around py-2">
          {mobileNavItems.map((item) => (
            <Link key={item.path} to={item.path}
              className={`flex flex-col items-center gap-0.5 px-3 py-1 text-xs transition-colors ${isActive(item.path) ? "text-primary" : "text-muted-foreground"}`}>
